...
def banner():
        ...
        print(f"{CYAN}        Fork by BenTheCyberOne{RESET}")
        print("")
banner()
...
def target_information(name,optHeaders):
    print("")
    print(f"{GREEN} [*] TARGET INFORMATION {RESET}")
    print("")
    try:
        if(optHeaders is not None):
            headers = requests.get(name,verify=False,headers=optHeaders)
        else:
...
def audit(optHeaders,target=[]):
        print("")
        for element in target:
                try:
                        if(optHeaders is not None):
                            handle = requests.get(element,verify=False,headers=optHeaders)
                        else:
...
def dump_credentials(dest,optHeaders):
        print("")
        pwd_targets = []
        ...
        for entry in pwd_targets:
                try:
                        if(optHeaders is not None):
                            handle = requests.get(entry,verify=False,headers=optHeaders)
                        else:
...
def fingerprint_frontpage(name,optHeaders):
        print("")
        ...
        for entry in build_enum_nix:
                try:
                        if(optHeaders is not None):
                            info = requests.get(entry,verify=False,headers=optHeaders)
                        else:
        ...
        try:
                if(optHeaders is not None):
                    version = requests.get(frontend_version,verify=False,headers=optHeaders)
                else:
...
def dump_sharepoint_headers(name,optHeaders):
        print ("")
        try:
                if(optHeaders is not None):
                    dump_s = requests.get(name,verify=False,headers=optHeaders)
                else:
        ...
        try:
                if(optHeaders is not None):
                    dump_f = requests.get(name,verify=False,headers=optHeaders)
                else:
        ...
        try:
                if(optHeaders is not None):
                    dump_g = requests.get(name,verify=False,headers=optHeaders)
                else:
...
def frontpage_rpc_check(name, optHeaders):
        print("")
        headers = {
                'MIME-Version': '4.0',
        ...
        try:
                if(optHeaders is not None):
                    response = requests.get(destination,verify=False,headers=optHeaders)
                else:
...
def frontpage_service_listing(name, optHeaders):
        print("")
        headers = {
                'MIME-Version': '4.0',
                'User-Agent': 'MSFrontPage/4.0',
...
def frontpage_config_check(name,optHeaders):
        print("")
        headers = {
                'MIME-Version': '4.0',
                'User-Agent': 'MSFrontPage/4.0',
...
def frontpage_remove_folder(name,optHeaders):
        print("")
        headers = {
                'MIME-Version': '4.0',
                'User-Agent': 'MSFrontPage/4.0',
...
def file_upload_check(name, optHeaders):
        print("")
        headers = {
                'MIME-Version': '4.0',
                'User-Agent': 'MSFrontPage/4.0',
...
def main():
        try:
            ...
            parser.add_argument("-H","--headers",help="Optional Headers - Ex: '{\"Host\":\"127.0.0.1\",\"Cookie\":\"abcdejghijklmnop\"}'",type=json.loads)

            args = parser.parse_args()
            target = args.url
            optHeaders = args.headers
            if(optHeaders is not None):
                print("Using these headers: ",optHeaders)
            target_information(target,optHeaders)
            if args.enumeration:
